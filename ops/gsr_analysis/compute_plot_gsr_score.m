...
load('ops/gsr_analysis/can_corr.mat');
load('ops/gsr_analysis/can_wrong.mat');
...
for k = 1:4
    ...
    gsr_resp = infer_gsr_resp(inds(k),can_corr,can_wrong);
    
    disp('Scores with the PE theory would be');
    score_pe = compute_score(subject,gsr_resp)
    s(k,3:4,1) = score_pe(1:2,1);
    s(k,3:4,2) = score_pe(1:2,2);
    
    subject.gsr.actions = gsr_resp;
    subject.gsr.perf_scores = score_pe;
    subject.gsr.csc_actions_neutral = gsr_resp(1,:)'==metadata.cues;
    subject.gsr.csc_actions_aversive = gsr_resp(2,:)'==metadata.cues;
...
a=plot(0:3,0.5*ones(4,1),':','color','k','linewidth',2); hold on;
legend('Behavioral responses','SCR responses')
uistack(a,'bottom');
...
%% Binomialtest
tr = myBinomTest(sum(sum(squeeze(s(:,3:4,2)))),8*150,0.5)
